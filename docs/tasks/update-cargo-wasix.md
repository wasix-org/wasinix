...
- `pkgs/toolchain/rust/cargo-wasix.nix`
- `pkgs/toolchain/rust/cargo-wasix.Cargo.lock`
...
2. Update `src.rev` and `src.hash` in `pkgs/toolchain/rust/cargo-wasix.nix`.
3. Refresh `pkgs/toolchain/rust/cargo-wasix.Cargo.lock` for that exact revision.
...
- Keep `pkgs/toolchain/rust/cargo-wasix.Cargo.lock` until upstream `cargo-wasix` commits a tracked `Cargo.lock` in the pinned revision.
