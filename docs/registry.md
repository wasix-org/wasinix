...
The wasix wheels PyPI cannot supply are published as a static PEP 503 "simple"
index: `.#legacyPackages.x86_64-linux.artifacts.registry.python`
(`pkgs/python-registry/`). `simple/` lists exactly the projects PyPI cannot
serve: those shipping a platform-tagged wheel, and those whose package unit
changes their source, version, patches, or build hooks (test-only changes do not
qualify). A pure, unmodified wheel is byte-for-byte PyPI's, so it is neither
stored nor listed here; a resolver takes this index as the priority beside PyPI
and gets our wheel for exactly the projects above, leaving every pure dependency
to PyPI at the version the consuming project asks for.
...
A new `pkgs/python/wheels/default.nix` entry lands in the registry automatically
when it is native or patched. Tests projected from the registry artifact walk
the index for hash and metadata integrity and pip-install representative
packages, resolving their dependencies beside PyPI, then import them under
Wasmer. The integrity walk resolves every served wheel's `Requires-Dist` naming
a project this index also serves, per interpreter the wheel installs on; a
requirement PyPI supplies is the resolve sweep's business instead.

The sandbox tests have no pypi.org, so `pkgs/python-registry/mirror-lock.json`
pins the pure wheels they need (regenerated by `update-mirror.py`, one universal
resolve per catalog project), and `mirror.nix` serves them on loopback beside
the registry. `python-registry-resolve-sweep` runs pip itself over every served
project against both, which catches what a model of pip cannot: a dependency
reached only through an extra, say. A project has to resolve on one interpreter,
not all of them, since a release states its own supported range.

Alongside `simple/`, the index root carries `packages.json`: one JSON object per
line naming a served wheel, which is how `wasmerio/wasmer-compat` decides which
projects the index covers.
...
pure-python wheels from PyPI directly. anybuild's template suite
...
```sh
pip install --index-url <preview>/simple --extra-index-url <prod>/simple <pkg>
```
