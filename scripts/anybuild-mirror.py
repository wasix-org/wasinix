...
"""Regenerate the PyPI pin set the hermetic anybuild template tests build against.
    ...
    nix run .#scripts.anybuild-mirror
...
against that release's examples/. update.py runs every hook whenever any target
moved, so this no-ops unless the recorded anybuild version changed; --force
re-resolves anyway.
"""
