...
exposePackage (
  let
    # the wheel's own interpreter, not pkgs.python3: this package is built once
    # per interpreter and the IAST extension links against these headers.
    python = packages.sameProfile.python;
    ...
    toolchainFile = pkgs.buildPackages.writeText "ddtrace-wasi-toolchain.cmake" ''
    ...
    '';
    updateWrapper = pkgs.buildPackages.writeShellApplication {
      name = "ddtrace-update";
      runtimeInputs = with pkgs.buildPackages; [curl git gnugrep gnused jq];
      text = ''
        exec bash "$(git rev-parse --show-toplevel)/pkgs/python-overlays/d/ddtrace/update.sh" "$@"
      '';
    };
  in
    packages.sameProfile.buildPythonPackage (finalAttrs: {
      ...
      # the wasix port (python/psutil). Building the vendored copy
      ...
      postPatch = ''
        ...
        install -Dm755 ${lib.getLib pkgs.libddwaf}/lib/libddwaf.so \
      ...
      build-system = with packages.sameProfile; [
        setuptools
        setuptools-scm
        cython
        setuptools-rust
      ...
      # cargoSetupHook explicitly, else pkgs.rustPlatform's wasm-cross one pulls
      ...
      nativeBuildInputs = [
        pkgs.buildPackages.rustPlatform.cargoSetupHook
