  ...
  let
    ...
    underBuildPerl = pkg:
      pkg.overrideAttrs (o: {
        preConfigure =
          (o.preConfigure or "")
          + "\nexport PATH=${packages.sameProfile.buildPackages.perl}/bin:$PATH\n";
      });

    underWasmer = substitution: pkg:
      pkg.overrideAttrs (o: {
        nativeBuildInputs = (o.nativeBuildInputs or []) ++ [packages.sameProfile.buildPackages.wasmer];
        postPatch = (o.postPatch or "") + "\n" + substitution;
      });
  in
    extendPackage (package.override {
      perlPackages = packages.sameProfile.perlPackages.overrideScope (_: pprev: {
        ...
        Test2Harness = packages.sameProfile.emptyDirectory;
        ...
        DBI = pprev.DBI.overrideAttrs (o: {
          postInstall =
            (o.postInstall or "")
            + ''
              autodir=$(echo $out/${packages.sameProfile.perl.libPrefix}/${packages.sameProfile.perl.version}/*/auto/DBI)
              printf 'sub dbd_dbi_arch_dir { return "%s"; }\n1;\n' "$autodir" \
                >> $out/${packages.sameProfile.perl.libPrefix}/cross_perl/${packages.sameProfile.perl.version}/DBI/DBD.pm
            '';
        });
      ...
      nixVersions = packages.sameProfile.nix.passthru.nixVersions;
      ...
      darcs = packages.sameProfile.emptyDirectory;
      breezy = packages.sameProfile.emptyDirectory;
      # only the test suite reads OPENLDAP_ROOT, and openldap reaches bash through
      # libtool, which builds at the off profile alone
      openldap = packages.sameProfile.emptyDirectory;
    }) {
      doCheck = false;
      nativeCheckInputs = _: [];
