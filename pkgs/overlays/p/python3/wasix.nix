    ...
    py =
      extendPackage (base.override {
        ...
        env = {
          ...
          LIBREADLINE_CFLAGS = "-I${lib.getDev packages.sameProfile.readline}/include";
        ...
        disallowedReferences = drf:
          builtins.filter (r: r != packages.sameProfile.buildPackages.bashNonInteractive)
          (
            if drf == null
            then []
            else drf
    ...
    python3 = _previous:
      result.python314.overrideAttrs (old: {
        passthru =
          (old.passthru or {})
          // {
            wasinix = ((old.passthru or {}).wasinix or {}) // {catalog = false;};
          };
      });
  };
in
