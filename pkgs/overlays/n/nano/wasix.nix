...
exposeWasixPackage (
  let
    nc = packages.sameProfile.ncurses;
  in
    wasmRename {wasmName = "nano";} (
      extendPackage (packages.sameProfile.callPackage "${packages.sameProfile.path}/pkgs/by-name/na/nano/package.nix" {
        enableNls = false;
        enableTiny = true;
        gettext = null;
        file = null;
