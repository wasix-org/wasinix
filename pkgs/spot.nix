...
# the entry point and the caveats; this file is only the wiring.
{
  ...
  # The three toolchains, and the default keep. Each enters outside pkgNames
  # (stdenv via replaceCrossStdenv, rustPlatform/haskellPackages as overlay
  # attrs), so the pin overlay has to name them explicitly to reach them.
  ...
  spotWith = {
    ...
    keepNames = keep;
    unknown = lib.filter (n: !(lib.elem n allNames)) keep;
    ...
    errors =
      lib.optional (unknown != []) "unknown keep name(s): ${lib.concatStringsSep ", " unknown}"
...
in {inherit spotWith toolchainNames;}
