...
[
  ...
  {attr = "envier";} # python/envier.nix (not in nixpkgs; ddtrace's config lib)
  ...
  {attr = "tornado";} # tornado/speedups.c (websocket xor masking); python/tornado.nix (no abi3, per-interpreter)
  {attr = "greenlet";}
  {attr = "gevent";} # python/gevent.nix (embedded libev, no libuv/c-ares)
  {attr = "uvloop";} # libuv; wasix/libuv/package.nix
  {
    attr = "zope-interface";
    pyImport = "zope.interface";
  }
  {attr = "caio";}
  {attr = "psutil";} # python/psutil (the linux backend minus what wasix lacks)
  {attr = "peewee";}
  {attr = "fastavro";}
  {attr = "zstandard";}
  {attr = "brotlicffi";}
  ...
  # python/tree-sitter-grammars (the ones nixpkgs lacks)
  {
    attr = "tree-sitter-c";
    publishOnce = true;
  }
  ...
  {attr = "yappi";}
  {attr = "zope-proxy";}
  {attr = "google-re2";} # re2 + abseil; python/google-re2.nix
  {attr = "onigurumacffi";}
  {
    attr = "pycocotools";
  ...
  {attr = "pycurl";} # curl; python/pycurl.nix
  {attr = "jq";} # jq + oniguruma
  {attr = "jqpy";}
  {attr = "pypandoc";} # spawns the wasm pandoc CLI; python/pypandoc.nix
  {
    attr = "pypandoc-binary";
    # dist is pypandoc_binary but the module stays pypandoc (as upstream).
    pyImport = "pypandoc";
  } # pypandoc + bundled wasm pandoc; python/pypandoc-binary.nix
  {attr = "apsw";} # sqlite
  {
    attr = "pynacl";
  ...
  } # renamed psycopg-c (pip's `psycopg[binary]`); python/psycopg-binary.nix
  {attr = "psycopg2";} # libpq via native pg_config splice; python/psycopg2.nix
  {
  ...
  } # zbar (the overlay's one shared lib); python/pyzbar.nix
  {attr = "shapely";} # geos; wasix/geos.nix
  {attr = "soundfile";} # libsndfile bundled into the wheel; python/soundfile.nix
  {attr = "sentencepiece";} # SWIG ext links wasix/sentencepiece.nix (static)
  {
    attr = "pyzmq";
    pyImport = "zmq";
  } # static libzmq; wasix/zeromq.nix + python/pyzmq (scikit-build-core)
  {attr = "duckdb";} # scikit-build-core; python/duckdb.nix (unity engine build)
  {attr = "h5py";} # cython ext links static libhdf5; wasix/hdf5.nix + python/h5py.nix
  {
    attr = "opencv-python";
    pyImport = "cv2";
  } # wasix/opencv4 + python/{opencv4.nix,opencv-python}
  {
    attr = "opencv-python-headless";
    pyImport = "cv2";
  } # python/opencv-python (same cv2, headless name)
  {attr = "snowflake-connector-python";} # bundled nanoarrow C++ result-parser ext; python/snowflake-connector-python.nix (dontCheckRuntimeDeps)
  {
    attr = "onnx";
    ...
    publishOnce = true;
  } # wasix/{protobuf,onnx} + ml-dtypes
  {
    attr = "onnxruntime";
    # the pybind11 inference engine, built per interpreter (it is not abi3).
    pyImport = "onnxruntime";
  } # wasix/onnxruntime + python/onnxruntime.nix
  {
    attr = "google-crc32c";
    pyImport = "google_crc32c";
  } # crc32c; wasix/crc32c.nix
  {
    attr = "mysqlclient";
    pyImport = "MySQLdb";
  } # libmysqlclient; wasix/mariadb-connector-c_3_3.nix
  {
    attr = "grpcio";
    pyImport = "grpc";
  } # python/grpcio.nix (vendored grpc core + abseil/boringssl/cares/re2/zlib)
  {
    attr = "pyarrow";
    pyImport = "pyarrow.parquet";
  } # arrow-cpp; python/pyarrow.nix

  # ── async / cython, no external C library ──────────────────────────────────────
  {attr = "aiohttp";} # vendored llhttp; deps multidict/yarl/frozenlist/aiosignal/…
  {attr = "httptools";} # cython + vendored llhttp; python/httptools.nix (llhttp __wasm__ guard)
  ...
  {attr = "rustworkx";}
  {attr = "burner-redis";}
  {attr = "cachebox";} # maturin/pyo3 (not in nixpkgs); python/cachebox.nix
  {
    attr = "dbt-extractor";
  ...
  } # maturin/pyo3 (dbt jinja parser)
  {
    ...
    skipTest = true;
    publishOnce = true;
  } # wasix/dbt-sa-cli + python/dbt-core-experimental-parser.nix
  {attr = "libcst";} # setuptools-rust (native CST parser)
  {
    attr = "hf-xet";
    pyImport = "hf_xet";
  } # maturin; python/hf-xet.nix (tokio/mio + wasm->browser cfg)
  {attr = "jiter";} # python/jiter.nix (maturin; anthropic/openai JSON core)
  {attr = "watchfiles";} # python/watchfiles.nix (maturin; uvicorn --reload watcher)
  {
    attr = "rpds-py";
    pyImport = "rpds";
  } # python/rpds-py.nix (maturin; jsonschema core)
  {attr = "ormsgpack";} # python/ormsgpack.nix (maturin; _PyLong_AsByteArray 3.13 abi fix)
  {attr = "tiktoken";} # python/tiktoken.nix (setuptools-rust; openai token counting)
  {
    attr = "uuid-utils";
    pyImport = "uuid_utils";
  } # python/uuid-utils.nix (maturin; getrandom 0.3/0.4 wasi vendor patch)
  {attr = "fastuuid";} # python/fastuuid.nix (maturin; getrandom wasi vendor patch)
  {attr = "tokenizers";} # python/tokenizers.nix (esaxx C++ + onig setjmp; legacy libc++ EH translated to exnref by the shared maturin hook, see WASIX-TODO.md)
  {attr = "bcrypt";} # python/bcrypt.nix (getrandom/target-lexicon forks)
  {
    attr = "pydantic-core";
    pyImport = "pydantic_core";
  } # python/pydantic-core.nix (maturin fork + getrandom + extension-module)
  {attr = "cryptography";} # python/cryptography.nix (maturin + openssl + target-lexicon dl)
  {attr = "primp";} # python/primp.nix (maturin; rquest/aws-lc-rs browser-impersonation HTTP client)
  {attr = "orjson";} # python/orjson.nix (maturin + target-lexicon dl)
  {attr = "ddtrace";} # python/ddtrace/package.nix (cython/C + IAST cmake + rust _native + bundled libddwaf)
