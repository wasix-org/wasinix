...
exposePackage (
  let
    qhullR = extendPackage pkgs.qhull {
      postInstall = ''
  ...
  in
    extendPackage (
      ...
      linkInputs (dropInputsByNameInfix ["ffmpeg"])
      // {
        passthru.wasinix.checks.captured.shards = 8;
        patches = _: [];
        mesonFlags = fs:
          builtins.filter (
            f: lib.versionAtLeast package.version "3.11" || !lib.hasInfix "system-libraqm" f
          ) (
            if fs == null
            then []
            else fs
        ...
        postPatch = ''
          ...
          ${lib.optionalString (lib.versionAtLeast package.version "3.11") ''
