...
in
  helpers.libTweaks {
    ...
    overrideAttrs = old: {
      ...
      passthru = (old.passthru or {}) // {wasix = (old.passthru.wasix or {}) // {preferredProfile = "off";};};
      # Eval everywhere (so preferredProfile is readable without building), but
      # refuse to BUILD outside off-EH.
      preConfigure =
        (old.preConfigure or "")
        + final.lib.optionalString (!offProfile) ''
      ...
      buildInputs = (old.buildInputs or []) ++ [final.ncurses];
      env =
        (old.env or {})
        // {
