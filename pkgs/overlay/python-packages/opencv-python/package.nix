...
# WITH_QT are off in overlay/packages/opencv4), which is what headless means, so
...
in {
  ...
  packages = {
    ...
    module = pyfinal.opencv4;
    inherit (module) version;
    py = pyfinal.python;
    # The tag the cross interpreter stamps on its own wheels
    # (_PYTHON_HOST_PLATFORM=wasix-wasm32, overlay/packages/python3); re-derive
