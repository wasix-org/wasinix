...
# "unknown" platform wasm32 maps to.
{
  ...
  result = exposeExtendedPackages (
    {
      icu = previous:
        result."icu${lib.versions.major previous.version}".overrideAttrs (old: {
          passthru =
            (old.passthru or {})
            // {
              wasinix =
                (old.passthru.wasinix or {})
                // {
                  update.post.syncAttrList = {
                    input = "nixpkgs";
    ...
    // lib.genAttrs (map (version: "icu${version}") versions) (_: tweak)
  );
in
